; (async () => {
  ...
  while (hasNextPage) {
      ...
      reviewItems.forEach(item => {
          const nameLinkEl = item.querySelector('a.y-css-1x1e1r2');
          ...
          const contentEl = item.querySelector('p[class*="comment"] span.raw__09f24__T4Ezm');
          const content = contentEl ? contentEl.innerText.trim() : "";

          const images = Array.from(item.querySelectorAll('img.y-css-xl4y6g'))
